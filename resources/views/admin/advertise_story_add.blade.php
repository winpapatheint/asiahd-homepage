<x-auth-layout>
    ...
    <section class="sec-padding">
        <div class="container_p">
            <h1>新しいストーリー登録</h1>
            <form method="POST" action="{{ route('admin.story.store') }}" id="storyRegisterForm" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    ...
                    <select name="section">
                        ...
                        @foreach ($sectionStories as $sectionStory)
                            <option value="{{ $sectionStory->id }}">{{ $sectionStory->section }}</option>
                        @endforeach
                    </select>
                ...
                <button type="button" onclick="validateStoryForm()">登録する</button>
            </form>
        </div>
    </section>
    <script>
        function validateStoryForm() {
            ...
            if (isValid) {
                document.getElementById('storyRegisterForm').submit();
            }
        }
    
        document.getElementById('storyRegisterForm').addEventListener('submit', function(event) {
